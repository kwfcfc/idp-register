...
GitHub receives only Forgejo's `stable` and `gh-pages` branches.

`gh-pages` is not a development branch. The Crow CI docs workflow
(`.crow/docs.jsonnet`) generates it from `stable` pushes and `vMAJOR.MINOR` release
tags, and GitHub Pages serves it. Do not commit to `gh-pages` manually; the next CI
publish or mirror sync may overwrite manual changes.
...
3. Set the branch filter to `stable,gh-pages`.
...
7. Enable GitHub Pages: Deploy from a branch, `gh-pages`, `/ (root)`.
...
`gh-pages` is an explicit exception to the one-commit-one-signature rule: its commits
are generated by the Crow CI docs workflow and are not signed. The branch's
trustworthiness derives from the signed `stable` and tag commits it is built from; each
publish commit records that source in `Source-Ref` / `Source-Commit` trailers in the
commit message body. Do not enable "require signed commits" branch protection on
`gh-pages`.
