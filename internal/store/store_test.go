...
// forEachStore runs fn once per database engine (AGENTS.md invariant #1):
// in-memory SQLite always, and PostgreSQL when TEST_POSTGRES_DSN points at a
// disposable database, e.g.
//
//	docker run --rm -d --name idp-register-test-pg -e POSTGRES_PASSWORD=test \
//	  -e POSTGRES_DB=idp_register_test -p 54329:5432 postgres:16-alpine
//	TEST_POSTGRES_DSN='postgres://postgres:test@localhost:54329/idp_register_test' \
//	  go test ./internal/store/
func forEachStore(t *testing.T, fn func(t *testing.T, s *Store)) {
	t.Run("sqlite", func(t *testing.T) {
		fn(t, openStore(t, &config.Config{DBDriver: config.DriverSQLite, DBDSN: "file::memory:?cache=shared"}))
	})
	t.Run("postgres", func(t *testing.T) {
		dsn := os.Getenv("TEST_POSTGRES_DSN")
		if dsn == "" {
			t.Skip("set TEST_POSTGRES_DSN to run the store suite against PostgreSQL")
		}
		resetPostgres(t, dsn)
		fn(t, openStore(t, &config.Config{DBDriver: config.DriverPostgres, DBDSN: dsn}))
	})
}
...
// from an empty database. The DSN must point at a disposable database.
